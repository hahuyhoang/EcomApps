...
export default function(data) {
    ...
    if(password_confirmation !== undefined) {
        let emtyValidationText = checkEmpty(password_confirmation, 'Please enter your password')

        if (emtyValidationText !== '') {
            return emtyValidationText;
        }else{
            let minLengthValidation = checkMinLength(password_confirmation, 3 , 'password')
            if(minLengthValidation !== '' ){
                return minLengthValidation
            }
        }
